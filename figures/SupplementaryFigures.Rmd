...
for(i in c(1:7)){
  ...
  p[[i]] <- 
    ...
    geom_text(data=df_freqPlot_pvalues %>% filter(IdentI %in% y), 
              inherit.aes = F, aes(y=height, x=Entity, label=p.adj), hjust=0.1, size=2.5, angle=45)+
...
df_freq_nh %>% 
  ...
  geom_text(data=pvalues, inherit.aes = F, aes(y=Max, x=Entity, label=p.adj), size=2.5)+
  scale_fill_manual(values = colors_nn)+
  scale_x_discrete(limits=c("rLN", "DLBCL", "MCL", "FL", "MZL"))+
