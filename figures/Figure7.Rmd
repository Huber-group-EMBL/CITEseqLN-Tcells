...
#ggsave(width = 8, height = 5.5, units = "cm", filename = "Figure7_p1.pdf")
...
#ggsave(width = 5, height = 6.1, units = "cm", filename = "Figure7_p3.pdf")
...
for(i in 1:length(pheat_nh$tree_row$order)) {
  ...
  plot_nn_rLN <- plot_nn_rLN+
    annotation_custom(grob = rectGrob(gp = gpar( fill=colors_nn[order_y][i], lex=1, col="white")), 
                      ...
                      xmin = 0, xmax = -1.1)+
    annotation_custom(grob = textGrob(label = paste0("N", order_y)[i], gp = gpar(cex=0.5)), 
                      ymin = seq(0.5, length(colors_nn)-0.5, 1)[i], 
                      ymax = seq(1.5, length(colors_nn)+0.5, 1)[i],
                      xmin = 0, xmax = -1.1)
...
plot_freq_nn <- df_freq_nh %>% 
  ...
  geom_text(data=pvalues, inherit.aes = F, aes(y=Y, x=Entity, label=p.adj), size=2.25)+
  ...
  ylab("% of total area")+
  mytheme_1+
  theme(strip.text.y = element_text(angle = 0, size=5.5, margin = unit(units = "cm", c(0.075,0.075,0.075,0.075))),
...
#ggsave(width = 19, height = 6, units = "cm", filename = "Figure7_p4.pdf")
...
plot_nn[[e]] <- selected %>% 
    ...
    geom_text(aes(x=2, y=1.08*Mean_new[1], label=round(Mean[1],1)), 
              check_overlap = T, size=2.25, color="black")+
    scale_color_manual(values = c("white", "black"), limits=c(F, T))+
    scale_fill_manual(values = colors_codex_exp)+
    scale_x_discrete(labels=unlist(labels_codex_exp))+
...
#ggsave(width = 19, height = 5, units = "cm", filename = "Figure7_p5.pdf")
