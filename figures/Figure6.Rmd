...
p1 <- 
  ...
  geom_text(inherit.aes = F, data = pvalues %>% filter(IdentI==6) %>% mutate(Y=c(38,45)),
            aes(x=Entity, y=Y, label=p), size=2.25)+
...
for(i in 5:9) {
  ...
  p3 <- p3+
    annotation_custom(grob = rectGrob(gp = gpar(fill=colors_umap_cl[as.character(cluster_order)[i]], col="white")), 
                    ...
                    xmin = 0, xmax = -0.75)+
    annotation_custom(grob = textGrob(label = labels_cl[[i]], rot = 0, hjust = 1, gp = gpar(cex=0.5)), 
                      ymin = rev(c(seq(0.5, 4.5, 1)+0.25))[i-4], 
                      ymax = rev(c(seq(1.5, 5.5, 1)-0.25))[i-4],
                    xmin = -1, xmax = -1)
}
...
#ggsave(width = 19, height = 5.8, units = "cm", filename = "Figure6_p1.pdf")
...
p4 <- 
  ...
  ggrepel::geom_text_repel(data=df_tmp1, aes(x=avg_log2FC, y=power, label=Label), show.legend = F, size=2.25, segment.size=0.25, xlim = c(-1.25, -1.75))+
  ggrepel::geom_text_repel(data=df_tmp2, aes(x=avg_log2FC, y=power, label=Label), show.legend = F, size=2.25, segment.size=0.25, xlim = c(1.3, 1.5))+
...
p5 <- ggplot()+
  geom_point_rast(data=DFtotal_5prime,
                  ...
                  alpha=ifelse(DFtotal_5prime$IdentI==11, 0.75, 0.15), stroke=0, shape=21)+
  ...
  geom_text(inherit.aes = F, aes(x=-0.75, y=8.5, label=label6), color=colors_umap_cl[["6"]], size=2.25)+
  geom_text(inherit.aes = F, aes(x=4.5, y=6.75, label=label14), color=colors_umap_cl[["14"]], size=2.25)+
  geom_text(inherit.aes = F, aes(x=8, y=1, label=label5), color=colors_umap_cl[["5"]], size=2.25)+
  ...
  theme(panel.border = element_rect(size=0.25),
        plot.title = element_textbox_simple(size = 6.5, width = NULL, padding = margin(1.25, 0, 1, 0), 
...
p6 <- df_freq %>% 
  ...
  stat_compare_means(comparisons = list(c("1/2", "3A")), vjust = -0.35, label.y = c(16.75),
                     size=2.25, tip.length = 0.02, bracket.size = 0.25)+
  ggtitle(expression('T'[REG]~'EM'[2]))+
  xlab("Grade")+
  ylim(0,18.25)+
...
#ggsave(width = 19, height = 6.5, units = "cm", filename = "Figure6_p2.pdf")
